...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{figures/Chapter 2/chapter2_hybridhydrogel.pdf}
    \caption{\textbf{Hybrid Hydrogel Concept.} Hybrid hydrogel concept integrating natural and synthetic components. This chapter introduces the development of a composite hydrogel system designed to mimic the structural and functional characteristics of native meniscal tissue. The hybrid approach combines a hydrated polymer network with synthetic fiber reinforcements, leveraging the biological advantages of natural hydrogels alongside the mechanical robustness of engineered scaffolds. Key aspects include optimizing polymer crosslinking, embedding synthetic meshes, and tailoring the composite architecture to achieve enhanced durability, anisotropy, and cell-instructive properties.}
    \label{fig:ch2_hybridhydrogel}
\end{figure}

\section{Defining ``Hybrid Hydrogel'': Natural + Synthetic Biomaterial Synergy}\label{sec:ch2_hybrid}

The successful development of biomaterials for meniscal tissue engineering requires meeting several critical criteria~\cite{patel2019systematicreview murphy20243dprintedvariable, bahcecioglu20193dprinted}. The scaffold must exhibit physiologically relevant water content and swelling behavior to maintain the osmotic environment necessary for cellular function~\cite{ahmed2015hydrogelpreparation,chaudhuri2016hydrogelstunable}.


Given these challenges, natural or composite natural–synthetic biomaterials offer a promising solution for engineering complex load-bearing tissues like the meniscus. The meniscus is a highly organized fibrocartilaginous structure, responsible for absorbing dynamic compressive loads and dissipating mechanical stress within the knee joint. Therefore, replicating both its mechanical and biological behavior is fundamental to the design of successful scaffold systems.

\section{Design Requirements and Approach}
\subsection{Mechanical and Biological Targets}
The development of biomaterials for meniscal tissue engineering requires meeting several critical criteria. The scaffold must exhibit physiologically relevant water content and swelling behavior to maintain the osmotic environment necessary for cellular function. It must possess mechanical properties that enable effective load distribution and stress dissipation under joint loading. Furthermore, it must support cellular adhesion, proliferation, and extracellular matrix production to facilitate biological integration and regeneration. Finally, structural stability under physiological conditions is essential to ensure that the scaffold maintains its form and function during prolonged in vivo use.

Specific design targets include:
\begin{itemize}
    \item Water Content: 70-80\% to match native tissue~\cite{morejon2020compressiveproperties}
    \item Matrix Distribution: Strategic PCL integration maintaining appropriate solid-to-fluid ratios
    \item Regional Variation: Composition gradients mimicking native tissue organization
    \item Structural Stability: Maintenance of form under physiological loading conditions
\end{itemize}

Given these requirements, natural or composite natural–synthetic biomaterials offer a promising solution. The meniscus's highly organized fibrocartilaginous structure, responsible for absorbing dynamic compressive loads and dissipating mechanical stress within the knee joint, demands scaffolds that can replicate both its mechanical and biological behavior.

\subsection{Polycaprolactone Integration: Mechanical Reinforcement and Biocompatibility}

While GelA-based hydrogels provide excellent biological properties and tunable swelling behavior, they often lack the mechanical strength required for load-bearing applications such as meniscal replacement \cite{2021synthesisapplication}. To address this limitation, polycaprolactone (PCL) was integrated as a synthetic reinforcement component \cite{bagherisaed2020functionalizedpoly}. PCL is a biodegradable aliphatic polyester with excellent mechanical properties, long-term structural stability, and established biocompatibility in orthopedic applications.

The integration of PCL reinforcement networks within the GelA hydrogel matrix creates a hybrid scaffold system that leverages the complementary properties of both materials. PCL provides mechanical strength and structural integrity, while the GelA hydrogel component maintains the physiological water content and biological signaling necessary for cellular function. Notably, the PCL framework serves as a mechanical backbone within which the GelA hydrogel is physically entrapped, promoting a synergistic interaction that leverages both materials' inherent strengths. This hybrid approach enables the development of composite scaffolds that better mimic the complex mechanical behavior of native meniscal tissue, including its anisotropic properties and region-specific variations in stiffness.

The role of PCL in mechanical reinforcement and its integration with GelA hydrogels will be explored in detail in subsequent sections, particularly as we examine the development of Hybrid Hydrogels Augmented via Network Integration (HANI) biomaterials. This systematic investigation will demonstrate how PCL networks can be strategically incorporated to enhance mechanical properties while maintaining the beneficial biological characteristics of the GelA matrix.

\begin{table}[H]
    \centering
    \caption{\textbf{PCL Material Properties.} Comprehensive overview of polycaprolactone (PCL) material properties relevant to scaffold design and fabrication, including mechanical, thermal, and biological characteristics.}
    \label{tab:PCL-properties}
    \begin{tabular}{|p{4.5cm}|p{4cm}|p{5.5cm}|}
        \hline
        \textbf{Property} & \textbf{Value (Typical)} & \textbf{Notes / References} \\[0.3em]
        \hline
        Chemical formula & (C$_6$H$_{10}$O$_2$)$_n$ & Aliphatic polyester \\[0.3em]
        \hline
        Molar mass (monomer) & 114.14 g/mol & -- \\[0.3em]
        \hline
        Density & 1.145--1.150 g/cm$^3$ & \cite{woodruff2010returnforgotten, pitt1981polycaprolactone} \\[0.3em]
        \hline
        Melting point & 58--63$^\circ$C & \cite{pitt1981polycaprolactone} \\[0.3em]
        \hline
        Glass transition temperature (Tg) & --60$^\circ$C & \cite{woodruff2010returnforgotten} \\[0.3em]
        \hline
        Crystallinity & 45--65\% & Semi-crystalline \cite{gunatillake2003polycaprolactone} \\[0.3em]
        \hline
        Young's modulus & 300--400 MPa & Depends on MW and crystallinity \cite{woodruff2010returnforgotten} \\[0.3em]
        \hline
        Tensile strength & 10--25 MPa & \cite{woodruff2010returnforgotten} \\[0.3em]
        \hline
        Elongation at break & 300--500\% & Highly ductile \cite{woodruff2010returnforgotten} \\[0.3em]
        \hline
        Flexural modulus & $\sim$400 MPa & -- \\[0.3em]
        \hline
        Hardness (Shore D) & $\sim$55 & Relatively soft \\[0.3em]
        \hline
        Print temperature (FDM) & 80--120$^\circ$C & Low-temperature thermoplastic \\[0.3em]
        \hline
        Decomposition temperature & $\sim$350$^\circ$C & Thermal degradation limit \\[0.3em]
        \hline
        Biodegradability & Months to years & Hydrolytic ester cleavage \cite{gunatillake2003polycaprolactone} \\[0.3em]
        \hline
        Biocompatibility & Excellent & Used in FDA-approved medical devices \cite{woodruff2010returnforgotten} \\[0.3em]
        \hline
        Degradation byproduct & Caproic acid & Non-toxic \\[0.3em]
        \hline
    \end{tabular}
\end{table}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{figures/Chapter 2/chapter2_biomaterials.pdf}
    \caption{\textbf{Overview of Natural and Synthetic Biomaterials Used in Scaffold Fabrication.} Porcine-derived gelatin A (GelA) is obtained through acid hydrolysis of collagen, preserving lysine amine groups essential for crosslinking. Glutaraldehyde (GTA), a dialdehyde crosslinker, facilitates network formation by reacting with amine groups on GelA chains. Poly-$\varepsilon$-caprolactone (PCL), a biodegradable polyester, serves as a synthetic reinforcement material, offering mechanical strength and ease of processing for composite scaffold integration. }
    \label{fig:chap2_biomaterials}
\end{figure}


\subsection{Gelatin A: Biochemical Properties and Mechanical Contributions}
...
\subsection{Glutaraldehyde Crosslinking: Chemical Mechanisms and Property Control}
...
\subsubsection{Swelling Analysis}

The swelling behavior varied significantly with both GTA concentration and volume:
\begin{itemize}
    \item Lower GTA concentrations (0.5\%) resulted in higher equilibrium swelling ratios
    \item Reduced crosslinker volumes (250~\(\mu\)L) allowed for greater water uptake
    \item Equilibrium was achieved within 12--24~hours for all formulations
\end{itemize}

Statistical analysis using two-way ANOVA revealed:
\begin{itemize}
    \item Significant main effects for both GTA concentration (p~<~0.001) and volume (p~<~0.001)
    \item No significant difference between 0.5\% and 1.5\% GTA at 0.25~mL (p~=~0.310)
    \item Effect size (\(\eta_p^2 = 0.096\)) indicating small-to-moderate concentration effects
\end{itemize}

\subsubsection{Water Content Analysis}
...
\begin{table}[H]
    \centering
    \caption{\textbf{Hydrogel Water Content Analysis.} Measured water content ($\phi_{WC}$) across hydrogel formulations with varying crosslinker concentrations and volumes, demonstrating the relationship between crosslinking parameters and hydration capacity.}
    \label{tab:ch2_water_content}
    \begin{tabular}{|c|c|c|c|}
        \hline
        \textbf{Volume (mL)} & \textbf{0.5\% GTA} & \textbf{1.5\% GTA} & \textbf{2.5\% GTA} \\[0.3em]
        \hline
        0.25 & 83.48\% $\pm$ 5.99\% & 75.62\% $\pm$ 5.49\% & 80.85\% $\pm$ 5.63\% \\[0.3em]
        \hline
        0.50 & 77.94\% $\pm$ 4.93\% & 77.49\% $\pm$ 5.46\% & 64.48\% $\pm$ 4.07\% \\[0.3em]
        \hline
        1.00 & 72.65\% $\pm$ 9.71\% & 70.55\% $\pm$ 6.58\% & 59.48\% $\pm$ 4.54\% \\[0.3em]
        \hline
    \end{tabular}
\end{table}
...
\subsubsection{Mechanical Testing Challenges and Development of Novel Approaches}
...
\subsubsection{Mechanical Testing Protocol}
...
\subsubsection{Co-Deposition with PCL Networks}
...
\begin{table}[H]
    \centering
    \caption{\textbf{Electrospinning Parameters and Conditions.} Detailed summary of key parameters and conditions used for gelatin fiber fabrication via electrospinning, including solution preparation, processing conditions, and post-treatment steps.}
    \label{tab:electrospinning_params}
    \begin{tabular}{|p{4.5cm}|p{7.5cm}|}
        \hline
        \textbf{Parameter} & \textbf{Value / Description} \\[0.3em]
        \hline
        Polymer Solution & 10\% w/v Gelatin A in HFIP \\[0.3em]
        \hline
        Solvent & Hexafluoroisopropanol (HFIP) \\[0.3em]
        \hline
        Syringe & 10~mL Luer-lock \\[0.3em]
        \hline
        Dispensing Needle & 23G tip \\[0.3em]
        \hline
        Voltage & 21~kV \\[0.3em]
        \hline
        Flow Rate & 2 and 3~mL/hr \\[0.3em]
        \hline
        Needle-Collector Distance & 15~cm \\[0.3em]
        \hline
        Collector Type & Vertical aluminum block with foil \\[0.3em]
        \hline
        Crosslinking Agent & 2.5\% Glutaraldehyde (30~min) \\[0.3em]
        \hline
        Post-Treatment & Glycine rinse + DI water wash \\[0.3em]
        \hline
        Fiber Analysis & SEM imaging + diameter measurement \\[0.3em]
        \hline
        Co-Deposition Method & Initial 2~mL gelatin electrospun; PCL grid placed; remainder electrospun over \\[0.3em]
        \hline
    \end{tabular}
\end{table}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/Chapter 2/chapter2_ES_sem.pdf}
    \caption{\textbf{SEM Analysis of Scaffold Components.} Placeholder for SEM images showcasing: (A) native GelA hydrogel microstructure; (B) electrospun gelatin fibers; (C) PCL grid structure; (D) hybrid co-deposited composite. Final images will highlight fiber morphology, pore distribution, and integration features.}
    \label{fig:ch2_ES_SEM}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{figures/Chapter 2/chapter2_earlyHANI.pdf}
    \caption{\textbf{Representative Early HANI Scaffold Prototypes.} Images showcase the initial generation of Hybrid Hydrogels Augmented via Additive Network Integration (HANI) scaffolds, including hydrogel constructs embedded with PCL frameworks of varied geometries and designs. Top row: examples of custom-shaped PCL reinforcements integrated into GelA–GTA hydrogels, demonstrating the flexibility of the composite system. Bottom row: scanning electron microscopy (SEM) images highlighting the microarchitecture of the PCL networks, illustrating porosity and interconnectivity critical for mechanical reinforcement and nutrient diffusion. }
