...
Recent advances in mechanical testing methodologies have provided new insights into the characterization of soft biomaterials~\cite{nelson2024methodstesting}. These approaches enable more accurate assessment of poroviscoelastic properties and better correlation with in vivo performance. The development of depth strategies for independent reinforcement has been particularly important for creating scaffolds with controlled mechanical properties and spatial organization~\cite{chendepthstraindependent}, allowing for the precise placement of reinforcement elements that can provide targeted mechanical support while maintaining biological functionality.

The distinction between confined and unconfined compression testing is particularly critical for understanding the mechanical behavior of hydrated biomaterials~\cite{bursaconnedunconned}. Confined compression testing, which restricts radial displacement and forces fluid flow axially, more accurately simulates the in vivo loading environment of meniscal tissue where anatomical constraints prevent lateral expansion. In contrast, unconfined compression allows radial expansion but may not capture the true poroelastic behavior relevant to meniscal function.

An in-depth guide to mechanical testing approaches for soft biomaterials reveals the complexity of establishing appropriate testing protocols~\cite{carterindepthguide}, particularly for materials with high water content and low mechanical stiffness. The choice of testing configuration, boundary conditions, and data analysis methods significantly influences the accuracy and relevance of mechanical characterization results. Understanding these testing fundamentals is essential for developing reliable protocols for hydrogel characterization.
...
\textbf{Equilibrium modulus} is derived from the steady-state stress after complete relaxation, representing the long-term mechanical behavior when fluid flow has ceased and only the solid matrix contributes to load bearing~\cite{buschermannconfinedcompression}.

\textbf{Permeability} characterizes the fluid transport properties of the material and can be calculated using different approaches depending on the testing configuration~\cite{mansour1976permeabilityarticular}:
...
Recent developments in 3D bioprinting have expanded the possibilities for creating complex tissue architectures~\cite{klarmann20243dbioprinting}. These technologies enable precise control over scaffold geometry and material distribution, allowing for the creation of biomimetic structures that closely replicate native tissue organization. The integration of digital light processing techniques has further enhanced the resolution and accuracy of additively manufactured components~\cite{hong2020digitallight}, enabling the fabrication of testing apparatus with features that would be impossible to achieve through traditional manufacturing methods.
...
The improved setup yielded aggregate modulus values of 15 kPa and 44 kPa for 2\% and 4\% agarose hydrogels, respectively, aligning well with literature values ranging from ~2--40 kPa for similar agarose concentrations\cite{buschmann1998mechanicalproperties}. Moreover, the setup exhibited trends consistent with literature as aggregate moduli and equilibrium forces increased with polymer concentration and cross-linking density for both hydrogels.
